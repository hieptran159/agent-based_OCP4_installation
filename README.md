...
```
cd mirror
sudo yum install -y podman httpd-tools openssl tmux \
...
```
echo 'export REGISTRY_SERVER=quay.hipe.id.vn' >> ~/.bashrc
source ~/.bashrc
sudo mkdir /quay
curl -sL https://developers.redhat.com/content-gateway/rest/mirror/pub/openshift-v4/clients/mirror-registry/1.3.11/mirror-registry.tar.gz -o - | tar xzvf -
...
```
echo 'REGISTRY_PW=7JXBTl9Y2KS35puf806UqmbWP4Gn1gzd' >> ~/.bashrc
source ~/.bashrc
```
...
Login vào registry
```
podman login --authfile pull-secret.txt -u init \
...
```
curl -L -s -o - https://mirror.openshift.com/pub/openshift-v4/x86_64/clients/ocp/stable/oc-mirror.tar.gz |sudo tar x -C /usr/local/bin -vzf - oc-mirror && sudo chmod +x /usr/local/bin/oc-mirror
```
...
```
oc-mirror --config=./imageset-config.yaml docker://$REGISTRY_SERVER:8443

oc-mirror --config=./imageset-config.yaml docker://$REGISTRY_SERVER:8443 --continue-on-error
```
...
```
export OPENSHIFT_INSTALL_RELEASE_IMAGE_OVERRIDE=quay.hipe.id.vn:8443/openshift/release-images@sha256:ede46c671ff7dcd2f9ccff821e24a319297529c9852d7e987ab268e22a21b782
```
_sha256 value_: 
...
~/mirror/openshift-install agent create image --log-level debug --dir cluster
```
...
Note-------------
	...
	echo 'REGISTRY_PW=CaEwnz5GM2ZxrNWBU4QX67bdL8F10c93' >> ~/.bashrc
